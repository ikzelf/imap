...
class LinkController extends BaseAjaxController
{
    ...
    {
        $hostsLinks = DB::find('hosts_links');
        $hostsLinksSettings = DB::find('hosts_links_settings');
    ...
    public function actionCreate(): array
    {
        ...
        foreach ($thostId as $targetHost) {
            if ($this->checkHostsIsWritable([$targetHost])) {
                ...
                $result = DB::find('hosts_links', $newLink);
                if (empty($result)) {
                    DB::insert('hosts_links', [$newLink]);
                    $result = DB::find('hosts_links', $newLink);
                }
    ...
    {
        $hostsLinks = DB::find('hosts_links', ['id' => $linkId]);
        $hostsLinksSettings = DB::find('hosts_links_settings', ['ids' => $linkId]);
    ...
    public function actionDelete(): array
    {
        ...
        DB::delete('hosts_links_settings', ['ids' => [$linkId]]);
        DB::delete('hosts_links', ['id' => [$linkId]]);
    ...
    public function actionUpdate(): array
    {
        ...
        DB::update('hosts_links', [$newLink]);
        DB::delete('hosts_links_settings', ['ids' => [$linkId]]);
        DB::insert('hosts_links_settings', [[
            'ids' => $linkId,
