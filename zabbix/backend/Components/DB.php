...
class DB
{
    ...
    const SCHEMA_FILE = 'config/schema.inc.php';
...
//					break;
    ...
    public static function getSchema($table = null)
    {
        if (is_null(self::$schema)) {
            self::$schema = include(realpath(__DIR__ . '/../../' . self::SCHEMA_FILE));
        }
     ...
     */
    public static function insert($table, $values, $getids = false)
    {
        if (empty($values)) {
            return true;
        }
     ...
     * @return array    an array of ids with the keys preserved
     */
    public static function insertBatch($table, $values, $getids = false)
    {
        if (empty($values)) {
            return true;
        }
