import StaffClusterIconCreator from '../../../services/map/staff-icon-creator';
...
class StaffClusterGroup extends L.MarkerClusterGroup {
    constructor() {
        super({
            maxClusterRadius: 30,
            spiderfyDistanceMultiplier: imapify.settings.spiderfyDistanceMultiplier,
            iconCreateFunction: StaffClusterIconCreator.build,
        });

        this.markerList = {};

        this.defaultLatLng = imapify.settings.startCoordinates;

        this.on('clustercontextmenu', (a) => {
            if ((a.layer._childCount < imapify.settings.maxMarkersSpiderfy) || (map.getMaxZoom() === map.getZoom())) {
                a.layer.spiderfy();
            } else {
